...
import com.ezerka.pingo.utility.FilePaths;
        ...
        if (mBytes.length / MB < MB_THRESHHOLD) {
            ...
            uploadTask.addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onFailure(@NonNull Exception exception) {
                    makeToast("Could not upload The Photo");
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (isValidDomain(mEmail.getText().toString())) {
                                ...
                                makeToast("Must use a company email");
                            }
        ...
        if (user != null) {
            user.sendEmailVerification()
                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Log.d(TAG, "OnComplete: Email sent for verification");
                                makeToast("Verification Email Sent");
                            } else {
                                makeToast("Couldn't Verify the email");
                                Log.d(TAG, "OnComplete: Unable to send the email" + task.getException());
                            }
                        }
                    });
        }
